...
First, the computer we're using doesn't have the graphical interface
...
* Follow the directions for students to [obtain an account](https://docs.unity.uri.edu/documentation/getting-access/) on URI's High Performance Computing Cluster (Unity).
...
* On the [main Unity page](https://unity.uri.edu/) select OpenOnDemand
* Select URI to log in
* At the top of the page select Shell - Unity Shell access

## Basic navigation

You are now logged in to Unity, which is a cluster of computers that can be used for research.
...
a alternatives for temporary and long-term storage.
This disk contains everyone's data, organized by research lab.
You change the directory using the `cd` command.
To get to the main folder for the lab you need to changed directories
...
Type `cd /project/pi_rsschwartz_uri_edu`.
...
Unlike your personal computer, Unity has multiple storage options.
This is because you will have extremely large datasets and for long-term storage these
should be stored in the most cost-effective way possible.
However, for more immediate work you will need storage drives with optimal processing speeds.
Because these drives have faster processing speeds they are also more expensive
and therefore you will have less space on them.
For this reason we may work on data in one place (faster speeds, but less space and more expensive) and then move our results to another location (more space, slower speed).
...
Read [Unity's storage guide](https://docs.unity.uri.edu/documentation/cluster_specs/storage/) for additional information.

Following these guidelines you should use `scratch` temporary space for running your analyses and `project` for long-term storage.

To request scratch space enter `ws_allocate -m your_email@uri.edu -r 1 -G pi_rsschwartz_uri_edu shared 30` (make sure to change the email address). This will allocate space for your use for 30 days. Remember to extend the expiration of the scratch space after 30 days or move completed work to the project storage - it will email you 1 day before.

Make sure to note the path to your storage folder.
